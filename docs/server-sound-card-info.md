...
This endpoint retrieves and returns information about all attached sound card devices on the machine this web-application is running on.
...
```
curl --location --request GET "http://your-server.com/server/sound-card-info?token=<TOKEN_VALUE>"
```
...
fetch("http://your-server.com/server/sound-card-info?token=<TOKEN_VALUE>", requestOptions)
...
```javascript
var settings = {
  "url": "http://your-server.com/server/sound-card-info?token=<TOKEN_VALUE>",
  "method": "GET",
  "timeout": 0
};
...
xhr.open("GET", "http://your-server.com/server/sound-card-info?token=<TOKEN_VALUE>");
...
var options = {
  ...
  'path': '/server/sound-card-info?token=<TOKEN_VALUE>',
  'maxRedirects': 20
};
...
curl_setopt_array($curl, [
  CURLOPT_URL => "http://your-server.com/server/sound-card-info?token=<TOKEN_VALUE>",
...
payload = ''
conn.request("GET", "/server/sound-card-info?token=<TOKEN_VALUE>", payload)
...
url = URI("http://your-server.com/server/sound-card-info?token=<TOKEN_VALUE>")
...
```javascript
{
   ...
   "data":[
      ...
      },
      {
         ...
         "vendor":"(Generic USB Audio)"
      }
   ],
   "time_generated":"2020-09-24T12:23:28-06:00"
}
```
...
| data | An array of [Sound Card Info](objects/sound-card-info.md) objects, each representing information about each sound card device on the system this application is running on | Array |
