...
This endpoint retrieves and returns information about all attached PCI and USB hardware devices on the machine this web-application is running on.
...
```
curl --location --request GET "http://your-server.com/server/hardware-info?token=<TOKEN_VALUE>"
```
...
fetch("http://your-server.com/server/hardware-info?token=<TOKEN_VALUE>", requestOptions)
...
```javascript
var settings = {
  "url": "http://your-server.com/server/hardware-info?token=<TOKEN_VALUE>",
  "method": "GET",
  "timeout": 0
};
...
xhr.open("GET", "http://your-server.com/server/hardware-info?token=<TOKEN_VALUE>");
...
var options = {
  ...
  'path': '/server/hardware-info?token=<TOKEN_VALUE>',
  'maxRedirects': 20
};
...
curl_setopt_array($curl, [
  CURLOPT_URL => "http://your-server.com/server/hardware-info?token=<TOKEN_VALUE>",
...
payload = ''
conn.request("GET", "/server/hardware-info?token=<TOKEN_VALUE>", payload)
...
url = URI("http://your-server.com/server/hardware-info?token=<TOKEN_VALUE>")
...
```javascript
{
   ...
   "data":[
      ...
      {
         "name":"USB 2.0 Webcam Device",
         "vendor":"Microsoft",
         "type":"USB"
      }
   ],
   "time_generated":"2020-09-22T16:02:52-06:00"
}
```
...
| data | An array of [Hardware Info](objects/hardware-info.md) objects, each representing information about each PCI and USB hardware device on the system this application is running on | Array |
