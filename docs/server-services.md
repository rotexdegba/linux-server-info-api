...
```
curl --location --request GET "http://your-server.com/server/services?token=<TOKEN_VALUE>"
```
...
fetch("http://your-server.com/server/services?token=<TOKEN_VALUE>", requestOptions)
...
```javascript
var settings = {
  "url": "http://your-server.com/server/services?token=<TOKEN_VALUE>",
  "method": "GET",
  "timeout": 0
};
...
xhr.open("GET", "http://your-server.com/server/services?token=<TOKEN_VALUE>");
...
var options = {
  ...
  'path': '/server/services?token=<TOKEN_VALUE>',
  'maxRedirects': 20
};
...
curl_setopt_array($curl, [
  CURLOPT_URL => "http://your-server.com/server/services?token=<TOKEN_VALUE>",
...
payload = ''
conn.request("GET", "/server/services?token=<TOKEN_VALUE>", payload)
...
url = URI("http://your-server.com/server/services?token=<TOKEN_VALUE>")
...
```javascript
{
   ...
   "data":[
      ...
      },
      {
         ...
         "description":"Adobe Flash Player Update Service",
         "loaded":1,
         "started":0,
         "state":"Stopped"
      },
   ...
   "time_generated":"2020-09-24T11:21:28-06:00"
}
```
...
| data | An array of [Service Info](objects/service-info.md) objects, each representing information about each service on the system this application is running on | Array |
